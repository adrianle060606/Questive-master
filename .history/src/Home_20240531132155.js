...
const Home = ({ accountInfo, questionID, course, questivians}) => {
  ...
  const convertChatDate = (chatDate) => {
    let text = "";
    if ((Date.now()-item.date) < 1000*60*2) {
      text = "Just Now"
    } else if ((Date.now()-item.date) < 1000*60*60) {
      text = `${Math.floor((Date.now()-item.date)/(1000*60))} minutes ago`; 
    } else if ((Date.now()-item.date) < 1000*60*60*24*2) {
      text = `${Math.floor((Date.now()-item.date)/(1000*60*60))} hours ago`;
    } else {
      text = `${Math.floor((Date.now()-item.date)/(1000*60*60*24))} days ago`;
    }
    return text;
  }
              ...
              <div id = "chat-content" class="chat-content">
                  {chatData.map((item) => (
                    <div class="message user-message">
                      ...
                      <span class="chat-time"> (convertChatDate(item.date)) </span>
