...
const Home = ({ accountInfo, questionID, course, questivians}) => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
    const q = query(collection(db, `${course === "Ext1" ? course : ""}chatRoom${questionID}`), orderBy("time"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const newData = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
            ...
            <div class="chat-messages" id="chat-messages">
              <div class="chat-content">
                <div class="message user-message">
                      <span class="chat-sender">Adrian:  </span>
                      <span class="chat-message html">Hey</span>
                </div>
              </div>
            </div>
